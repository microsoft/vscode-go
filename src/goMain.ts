...
export function activate(ctx: vscode.ExtensionContext): void {
	let useLangServer = vscode.workspace.getConfiguration('go')[
		'useLanguageServer'
	];
	...
	updateGoPathGoRootFromConfig().then(() => {
		...
		if (prevGoroot !== currentGoroot && prevGoroot) {
			vscode.window
				.showInformationMessage(
					'Your goroot is different than before, a few Go tools may need recompiling',
					updateToolsCmdText
				)
				.then(selected => {
			...
			getGoVersion().then(currentVersion => {
				if (currentVersion) {
					...
					const currVersionString = `${currentVersion.major}.${
		...
		if (checkLanguageServer()) {
			const languageServerExperimentalFeatures: any =
				vscode.workspace
					.getConfiguration('go')
					.get('languageServerExperimentalFeatures') || {};
			let langServerFlags: string[] =
			...
			const c = new LanguageClient(
				...
				},
				{
					initializationOptions: {
						funcSnippetEnabled: vscode.workspace.getConfiguration(
							'go'
						)['useCodeSnippetsOnFunctionSuggest'],
						gocodeCompletionEnabled:
							languageServerExperimentalFeatures['autoComplete']
					},
					documentSelector: ['go'],
					uriConverters: {
					...
					middleware: {
						provideDocumentFormattingEdits: (
							...
							if (
								languageServerExperimentalFeatures['format'] ===
								true
							) {
						...
						provideRenameEdits: (
							...
							if (
								languageServerExperimentalFeatures['rename'] ===
								true
							) {
						...
						provideHover: (
							...
							if (
								languageServerExperimentalFeatures['hover'] ===
								true
							) {
								return next(document, position, token);
							}
			...
			c.onReady().then(() => {
				const capabilities =
					c.initializeResult && c.initializeResult.capabilities;
				if (!capabilities) {
				...
				if (
					languageServerExperimentalFeatures['format'] !== true ||
				...
				if (
					languageServerExperimentalFeatures['rename'] !== true ||
				...
				if (
					languageServerExperimentalFeatures['goToTypeDefinition'] !==
						true ||
				...
				if (
					languageServerExperimentalFeatures['hover'] !== true ||
				...
				if (
					languageServerExperimentalFeatures['goToDefinition'] !==
						true ||
				...
				if (
					languageServerExperimentalFeatures['findReferences'] !==
						true ||
				...
				if (
					languageServerExperimentalFeatures['documentSymbols'] !==
						true ||
				...
				if (
					languageServerExperimentalFeatures['signatureHelp'] !==
						true ||
				...
				if (
					languageServerExperimentalFeatures['workspaceSymbols'] !==
						true ||
				...
				if (
					languageServerExperimentalFeatures['goToImplementation'] !==
						true ||
	...
	buildDiagnosticCollection = vscode.languages.createDiagnosticCollection(
		'go'
	);
	...
	vetDiagnosticCollection = vscode.languages.createDiagnosticCollection(
		'go-vet'
	);
	...
	ctx.subscriptions.push(
		vscode.commands.registerCommand('go.gopath', () => {
			...
			let wasInfered = vscode.workspace.getConfiguration(
				'go',
				vscode.window.activeTextEditor
					? vscode.window.activeTextEditor.document.uri
					: null
			...
			if (
				...
				root = vscode.workspace.getWorkspaceFolder(
					vscode.window.activeTextEditor.document.uri
				).uri.fsPath;
	...
	ctx.subscriptions.push(
		vscode.commands.registerCommand('go.test.cursor', args => {
			const goConfig = vscode.workspace.getConfiguration(
				'go',
				vscode.window.activeTextEditor
					? vscode.window.activeTextEditor.document.uri
					: null
	...
	ctx.subscriptions.push(
		vscode.commands.registerCommand('go.test.package', args => {
			let goConfig = vscode.workspace.getConfiguration(
				'go',
				vscode.window.activeTextEditor
					? vscode.window.activeTextEditor.document.uri
					: null
	...
	ctx.subscriptions.push(
		vscode.commands.registerCommand('go.import.add', (arg: string) => {
	...
	ctx.subscriptions.push(
		vscode.commands.registerCommand('go.tools.install', args => {
	...
	ctx.subscriptions.push(
		vscode.workspace.onDidChangeConfiguration(
			(e: vscode.ConfigurationChangeEvent) => {
				...
				let updatedGoConfig = vscode.workspace.getConfiguration(
					'go',
					vscode.window.activeTextEditor
						? vscode.window.activeTextEditor.document.uri
						: null
				...
				if (
					...
					vscode.window
						.showInformationMessage(
							'Reload VS Code window for the change in usage of language server to take effect',
							'Reload'
						)
						.then(selected => {
							if (selected === 'Reload') {
...
function didLangServerConfigChange(
	e: vscode.ConfigurationChangeEvent
): boolean {
	return (
